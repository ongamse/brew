...
module Homebrew
  module Cmd
    class Bundle < AbstractCommand
      ...
      def self.to_english_list(items)
        case items.length
        when 0
          ""
        when 1
        ...
        when 2
          ...
          "#{items.first(items.length - 1).join(", ")} and #{items.last}"
        end
      end
      ...
      def self.extension_dump_disabled?(args, extension)
        predicate_method = extension.dump_disable_predicate_method
        return false if predicate_method.nil?

        args.public_send(predicate_method)
      end

      BUNDLE_EXTENSIONS = T.let(Homebrew::Bundle.extensions.freeze, T::Array[T.class_of(Homebrew::Bundle::Extension)])
 ...
 "`--#{extension.flag}`"
      ...
      BUNDLE_REMOVE_FLAGS_DESCRIPTION = T.let(
        to_english_list(
           ...
           *BUNDLE_EXTENSIONS.select(&:remove_supported?).map { |extension| "`--#{extension.flag}`" }],
        ).freeze,
        String,
      )
      ...
      def run
        ...
        when "dump"
          ...
          Homebrew::Bundle::Commands::Dump.run(
            ...
            cargo:,
            flatpak:,
            **BUNDLE_EXTENSIONS.select(&:dump_supported?).to_h do |extension|
